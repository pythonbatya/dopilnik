...
> **dopilnik fork** of [Simply Blame](https://github.com/JamiLu/simply-blame) (upstream v1.11.2, GPL-2.0).
> Differences from upstream: publisher is `lvinogradov` (marketplace version can never auto-install over
> this build), no contributed keybindings — the toggle hotkey (`cmd+;`) lives in the user's keybindings.json.
> Upstream README below, as-is.
...
* **Hash Action**
   * Select an action to be invoked when the commit hash is clicked on the hover box. Supported actions are copy to clipboard, open in browser, and open in VS Code.
* **Ignore White Space**
