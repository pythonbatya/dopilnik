...
import { blameFile, blameRevFile } from './Git';
import { REV_SCHEME, parseRevUri } from './RevDiff';
...
const blameDocument = async (document: vscode.TextDocument): Promise<string> => {
    if (document.uri.scheme === REV_SCHEME) {
        const { repoRoot, rev, relPath } = parseRevUri(document.uri);
        if (!rev || document.getText().length === 0) {
            return '';
        }
        return blameRevFile(repoRoot, rev, relPath);
    }
    if (document.uri.scheme !== 'file') {
        // VSCode-internal schemes (e.g. the `git:` side of a working-tree
        // diff) carry no real filesystem name — no column there, silently.
        return '';
    }
    return blameFile(document.fileName);
};

export const blame = async (document: vscode.TextDocument): Promise<BlamedDocument[]> => {
  	const blamed = (await blameDocument(document)).split("\n");
